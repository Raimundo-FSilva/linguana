...
const Selector = ({action, selected,  index, label}: {action: (data: string) => void, selected: boolean, index: string, label: string}) => {
  ...
  useEffect(() => {
    context.setDescriptionQuest("Selecione a palavra correspondente:")
  })
...
const Card = ({value, options}: {value: string | null, options: { label: string, value: string }[]}) => {
  ...
  return (
    <span
      ...
      className={`bg-[#F7F7F7] px-3 text-[#00CC2D] break-all py-[4px] relative text-xl font-semibold items-center rounded select-none`}>
      {responseWord?.label}
    </span>
  )
}
  ...
  return (
    <div className={"flex max-sm:flex-col max-sm:items-center py-10 gap-10 items-center h-full max-w-[550px] mx-auto"}>
      ...
      <div className={"flex flex-col gap-8"}>
                ...
                .map(
                  (word, index) =>  
                   word.includes("$data-") ? 
                    ...
                    <span 
                      key={index} 
                      className={"text-2xl max-sm:text-xl  select-none font-semibold font-sora text-[#3B403C]"}>
                      {word}
                    </span>
                  )
            }
        ...
        <ul className={"flex font-sora gap-3 max-sm:justify-center"}>
          {
